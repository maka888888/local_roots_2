...
class FarmerFarmMyFarmEditCertificates extends ConsumerStatefulWidget {
  final AppUserModel appUser;
  const FarmerFarmMyFarmEditCertificates({super.key, required this.appUser});

  @override
  FarmerFarmMyFarmEditCertificatesState createState() =>
      FarmerFarmMyFarmEditCertificatesState();
}

class FarmerFarmMyFarmEditCertificatesState
    extends ConsumerState<FarmerFarmMyFarmEditCertificates> {
  ...
  Future _selectPicture() async {
    ...
    await uploadFarmPhoto(context, ImageSource.gallery).then((value) {
      if (value != null) {
        setState(() {
          _farmer.certificatesPhotos.add(value);
          _isSaving = false;
        });
      }
    });
          ...
          children: [
            ...
            _isSaving
                ...
                : SizedBox(
                    height: 30,
                    child: Text(
                        AppLocalizations.of(context)!.uploadRelatedCertificates,
            ...
            if (_farmer.certificatesPhotos.isNotEmpty)
              GridView.builder(
                shrinkWrap: true,
                itemCount: _farmer.certificatesPhotos.length,
                ...
                itemBuilder: (context, index) {
                  return Stack(
                    children: [
                      Positioned.fill(
                        child: Image.network(
                          _farmer.certificatesPhotos[index],
                          fit: BoxFit.cover,
                        ),
                      ),
                      Positioned(
